...
export default function CategoryBar({ active, onChange, count, categories = DEFAULT_CATEGORIES }) {
  return (
    <section className="relative z-[1] px-3 pb-4 pt-7 sm:px-[4vw] sm:pb-6 sm:pt-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div
          className="flex flex-1 gap-[3px] overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] rounded-full border border-brand/20 p-[3px] backdrop-blur-[8px]"
        ...
        >
          {categories.map((cat) => (
            <button
              ...
              className={`catTab flex-1 shrink-0 whitespace-nowrap rounded-full border-none bg-transparent px-3 py-[.55rem] font-sans text-[.52rem] uppercase tracking-[.1em] cursor-pointer transition-[color_.25s,background_.25s,transform_.2s] sm:py-[.62rem] sm:text-[.6rem] sm:tracking-[.18em] sm:px-5 hover:bg-brand/[.08] hover:text-ink${active === cat.id ? " active bg-brand text-cream" : " text-muted"}`}
